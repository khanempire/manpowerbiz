...
export default function PaymentForm({ clientSecret, paymentValue, setExternalElements, externalElements, externalStripe, setExternalStripe, dataToSend, nextStep, prevStep, invoiceData, setInvoiceData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    //   }
    // } else {
    let apiResponse = await postCallWithHeaders("admin/makePayment", dataToSend);
    ...
    if (apiResponse?.error) {
      ...
      // nextStep();
      navigate("/admindashboard/viewPayments");
  ...
  return (
    <form id="payment-form" onSubmit={handleSubmit}>
      ...
      <div>
        ...
        <InvoiceViewCard data={invoiceData} />
      </div>

      <Group position="apart" style={{ marginTop: "1rem" }}>
        <Button variant="filled" onClick={prevStep} leftIcon={<IconArrowLeft />}>
          Back
        </Button>
        <button
        ...
        >
          <span id="button-text">{isLoading ? <div className="spinner" id="spinner"></div> : "Complete Payment"}</span>
        </button>
      </Group>
